...
//    private HashSet<Descriptor> siblings = null;
    ...
    // The previous hosts
    private ArrayList<Descriptor> previousHostsAlive; // A list with the PHs still avai;able i the current version
...
//                        }
                ...
                for(String dui : previousSCR.getDescriptorUIs()){
                    Descriptor pseudoHost = h.getDescriptorByID(dui);
                    if(pseudoHost != null){
                        previousHostsAlive.add(pseudoHost);
                    }
                }  
            ...
            if( h.descriptorExists(host)){                
                previousHostsAlive.add(h.getDescriptorByID(host.getDescriptorUI()));
            }
...
//                otherHosts.removeAll(siblingHosts);
     ...
     */
    public String getPreviousHostsUI(){
        ...
        for(int i=0; i<=PHs.size()-1; i++){
            if(this.descriptorExists(PHs.get(i))){
                if(PHUIs==""){
                    PHUIs = descriptorNowTerms.get(PHs.get(i)).getDescriptorUI();
                }
                else{
